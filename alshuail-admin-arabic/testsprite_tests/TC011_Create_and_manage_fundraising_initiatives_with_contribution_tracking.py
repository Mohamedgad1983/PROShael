...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click login button to authenticate as initiative manager
        ...
        # -> Retry login by clicking login button again or check for error messages
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Fundraising Initiative Successfully Created').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test plan execution failed: Creation and validation of charitable initiatives, including Diya cases, donation campaigns, and medical assistance, did not complete successfully.')
