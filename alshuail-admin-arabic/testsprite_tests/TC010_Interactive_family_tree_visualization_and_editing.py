...
async def run_test():
    ...
    try:
        ...
        # -> Check if JavaScript is enabled or try to reload page or report issue
        frame = context.pages[-1]
        # Retry login button click to see if JavaScript error persists
        elem = frame.locator('xpath=html/body/div/div/main/div/div[2]/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        # Click login button to attempt login with filled credentials
        elem = frame.locator('xpath=html/body/div/div/main/div/div[2]/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Family Tree Loaded Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test plan execution failed: The interactive family tree did not display all 10 tribal branches correctly, or zoom, pan, and detailed member relationship views are not functioning as expected.")
