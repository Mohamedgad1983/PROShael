...
async def run_test():
    ...
    try:
        ...
        # -> Retry login by clicking login button again
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Payment Accepted').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Payment below minimum allowed amount was not rejected as expected. The error message indicating rejection was not displayed.')
