...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Re-enter email admin@alshuail.com
        elem = frame.locator('xpath=html/body/div/div/main/div/div[2]/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('admin@alshuail.com')
        

        frame = context.pages[-1]
        # Re-enter password Admin@123
        elem = frame.locator('xpath=html/body/div/div/main/div/div[2]/form/div[2]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Admin@123')
        ...
        # -> Check network or console logs for errors, or try to reload page and retry login. If no success, report issue.
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError("Test case failed: Recording a payment using different methods (KNET, bank transfer) including uploading payment receipt documents, and updating payment status with overdue alerts did not pass as expected.")
