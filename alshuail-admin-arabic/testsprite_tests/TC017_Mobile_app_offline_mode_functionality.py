...
async def run_test():
    ...
    try:
        ...
        # -> Click login button to authenticate and load dashboard
        ...
        # -> Fill email and password fields with valid credentials and click login button
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click تسجيل الدخول button to submit login form
        elem = frame.locator('xpath=html/body/div/div/main/div/div[2]/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Offline Subscription Access Enabled').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test plan execution failed: Offline viewing and queueing actions such as viewing subscriptions or family tree do not work without network connectivity as expected.')
