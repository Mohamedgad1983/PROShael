...
staging_dir="$(mktemp -d /tmp/goods-financing-fees.XXXXXX)"
cleanup() {
  rm -rf "$staging_dir"
}
trap cleanup EXIT
...
for file in "${files[@]}"; do
  install -d -m 700 "$staging_dir/$(dirname "$file")"
  curl --fail --silent --show-error --location \
    "$payload_root/$file" \
    --output "$staging_dir/$file"
done

grep -Fq 'Object.freeze({ principal: 3000, fee: 500 })' \
  "$staging_dir/src/services/familyFinancingPolicy.js"
grep -Fq 'Object.freeze({ principal: 6000, fee: 800 })' \
  "$staging_dir/src/services/familyFinancingPolicy.js"
grep -Fq 'Object.freeze({ principal: 10000, fee: 1400 })' \
  "$staging_dir/src/services/familyFinancingPolicy.js"
grep -Fq 'تمويل السلع' "$staging_dir/src/services/loanService.js"
grep -Fq '"fee":1400' "$staging_dir/migrations/20260823_update_goods_financing_fees.sql"

for file in "${files[@]}"; do
  case "$file" in
    *.js) node --check "$staging_dir/$file" ;;
  esac
done
...
for file in "${files[@]}"; do
  install -d -m 755 "$app_dir/$(dirname "$file")"
  install -m 644 -o alshuail -g alshuail "$staging_dir/$file" "$app_dir/$file"
done
...
wait_for_health
grep -Fq 'Object.freeze({ principal: 3000, fee: 500 })' \
  "$app_dir/src/services/familyFinancingPolicy.js"
