...
} from './financingRepaymentService.js';
import {
  FAMILY_FINANCING_POLICY_VERSION,
...
const STATUS_NOTIFICATIONS = {
  [LOAN_STATUS.UNDER_FUND_REVIEW]: {
    title: 'جاري مراجعة طلب تمويل السلع',
    body: (loan) => `تم استلام طلب تمويل السلع رقم ${loan.sequence_number} وجاري مراجعته من الصندوق.`,
  },
  [LOAN_STATUS.APPROVED_BY_FUND]: {
    title: 'تمت الموافقة المبدئية',
    body: (loan) => `تمت الموافقة على طلب تمويل السلع رقم ${loan.sequence_number}. سيتم تحويل الطلب للمؤسسة لاستكمال الإجراءات.`,
  },
  [LOAN_STATUS.FORWARDED_TO_BROUJ]: {
    title: 'تم تحويل الطلب للمؤسسة',
    body: (loan) => `تم تحويل طلب تمويل السلع رقم ${loan.sequence_number} لمؤسسة بروز الريادة لاستكمال الإجراءات عبر منصة ناجز.`,
  },
  ...
  [LOAN_STATUS.READY_FOR_DISBURSEMENT]: {
    title: 'جاهز للصرف',
    body: (loan) => `طلب تمويل السلع رقم ${loan.sequence_number} جاهز للصرف من الصندوق.`,
  },
  [LOAN_STATUS.COMPLETED]: {
    title: 'تم صرف تمويل السلعة',
    body: (loan) => `تم صرف تمويل السلعة للطلب رقم ${loan.sequence_number} وتفعيل جدول الأقساط.`,
  },
  [LOAN_STATUS.REJECTED]: {
    title: 'تم رفض طلب تمويل السلع',
    body: (loan) => {
      const reason = loan.rejection_reason ? ` السبب: ${loan.rejection_reason}.` : '';
      return `تم رفض طلب تمويل السلع رقم ${loan.sequence_number}.${reason} للاستفسار يرجى التواصل مع إدارة الصندوق.`;
    },
  },
};
...
export async function dispatchStatusNotification(loan, toStatus) {
  ...
  if (!template) {return;}
  try {
    await createMemberNotification(loan.member_id, {
...
export async function getLoanSettings() {
  ...
  if (rows.length === 0) {
    return {
      ...
      financing_tiers: DEFAULT_FINANCING_TIERS,
...
export async function checkLoanEligibility(memberId) {
  ...
  if (!settings.enabled) {
    return {
      ok: false,
      code: 'LOAN_DISABLED',
      message: 'خدمة تمويل السلع غير متاحة حالياً',
  ...
  return {
    ok: true,
    code: 'ELIGIBLE',
    message: 'يمكنك التقديم على تمويل السلع',
    ...
    settings: {
      ...
      financing_tiers: normalizeFinancingTiers(settings.financing_tiers),
      default_installment_count: Number(settings.default_installment_count || 10),
      max_installment_count: Number(settings.max_installment_count || 12),
...
export async function validateRequestPayload(payload) {
  ...
  try {
    resolveFinancingTier(requestedItemAmount, settings.financing_tiers, { requireExact: true });
  } catch (error) {
    return {
...
export async function createLoanRequest({ memberId, payload }) {
  ...
  try {
    ...
    const requestedItemAmount = requestedItemAmountFromPayload(payload);
    const tier = resolveFinancingTier(requestedItemAmount, settings.financing_tiers, { requireExact: true });
    const feeRate = tier.fee / tier.principal;
    ...
    const insert = await client.query(
      ...
      [
        ...
        Number(payload.monthly_salary), Number(payload.monthly_obligations), requestedItemAmount, tier.principal,
        feeRate, 1,
        tier.fee, tier.total, JSON.stringify({
          policy_version: FAMILY_FINANCING_POLICY_VERSION,
          terms_version: FAMILY_FINANCING_TERMS_VERSION,
          terms_text_ar: FAMILY_FINANCING_TERMS_AR,
