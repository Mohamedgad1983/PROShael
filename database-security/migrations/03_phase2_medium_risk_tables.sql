-- =====================================================
-- PHASE 2: MEDIUM-RISK TABLES - RLS ENABLEMENT
-- =====================================================
...
-- Column assumptions: user_id or member_id exists
...
WITH CHECK (
    ...
    OR (auth.jwt() ->> 'member_id' = member_id::text)
...
USING (
    (auth.uid()::text = user_id::text)
    OR (auth.jwt() ->> 'member_id' = member_id::text)
    OR (auth.jwt() ->> 'id'::text = member_id::text)
    OR (auth.jwt() ->> 'role' = 'admin')
);
...
'RLS Enabled: Public read, Admin write - Phase 2';
