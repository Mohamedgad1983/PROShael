/**
 * InitiativePayment - صفحة المساهمة في المبادرات
 * 
 * Features:
...
const InitiativePayment = () => {
  ...
  const { user } = useAuth()
  const { invalidateCache, fetchInitiatives } = useDataCache()
  ...
  const loadInitiatives = async () => {
    try {
      ...
      if (result?.data && result.data.length > 0) {
        ...
        if (response?.initiatives || response?.data) {
          setInitiatives(response.initiatives || response.data)
        } else {
          // Mock data
          setInitiatives([
            {
              id: '1',
              title_ar: 'مبادرة زواج الشباب',
              description_ar: 'مساعدة الشباب في تكاليف الزواج',
              target_amount: 50000,
            ...
            {
              id: '2',
              title_ar: 'مبادرة دعم الطلاب',
              description_ar: 'دعم الطلاب المتفوقين في الجامعات',
              target_amount: 30000,
  ...
  const handlePayment = async () => {
    ...
    try {
      const paymentData = {
        ...
        notes: `مساهمة في ${selectedInitiative.title_ar}`
      }

      const response = await paymentService.payInitiative(paymentData)
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-emerald-500 to-green-600 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl p-8 text-center max-w-sm w-full shadow-2xl animate-scaleIn">
          ...
          <div className="bg-gray-50 rounded-xl p-4 mb-6 text-right">
            ...
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-gray-500">المبادرة</span>
              <span className="font-bold text-gray-800 text-sm">{selectedInitiative?.title_ar}</span>
            </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">جاري تحميل المبادرات...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      {/* Header */}
      <div className="bg-gradient-to-r from-emerald-500 to-green-600 px-5 pt-14 pb-8">
        <div className="flex items-center mb-4">
          <button onClick={() => navigate(-1)} className="back-button ml-3">
            <ArrowRight size={20} />
          </button>
          <div>
            <h1 className="text-xl font-bold text-white">المساهمة في المبادرات</h1>
            <p className="text-white/70 text-sm">اختر المبادرة والمبلغ</p>
          </div>
        </div>
      ...
      <div className="px-5 -mt-4">
        ...
        <div className={`rounded-xl p-4 mb-5 flex items-center gap-3 ${
          ...
          <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
            isOnBehalf ? 'bg-pink-500' : 'bg-emerald-500'
          }`}>
            {isOnBehalf ? <Users size={24} className="text-white" /> : <User size={24} className="text-white" />}
          </div>
          <div>
            <p className={`text-sm ${isOnBehalf ? 'text-pink-600' : 'text-emerald-600'}`}>
          ...
          {initiatives.length > 0 ? (
            <div className="space-y-3">
              {initiatives.map(initiative => {
                const progress = getProgress(initiative.current_amount, initiative.target_amount)
                ...
                return (
                  <button
                    ...
                    className={`w-full text-right p-4 rounded-xl border-2 transition-all ${
                      isSelected
                        ? 'bg-gradient-to-r from-emerald-50 to-green-50 border-emerald-400 shadow-lg'
                        : 'bg-white border-gray-200 hover:border-emerald-200'
                    }`}
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h4 className="font-bold text-gray-800">{initiative.title_ar}</h4>
                        <p className="text-gray-500 text-sm">{initiative.description_ar}</p>
                      </div>
                      {isSelected && (
                    ...
                    <div className="flex justify-between text-sm">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-green-600">{initiative.current_amount.toLocaleString('ar-SA')} ر.س</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Target size={14} className="text-gray-400" />
                        <span className="text-gray-500">{initiative.target_amount.toLocaleString('ar-SA')} ر.س</span>
                      </div>
                    </div>
            ...
            <div className="bg-gray-100 rounded-xl p-6 text-center">
              ...
              <p className="text-gray-500">لا توجد مبادرات نشطة حالياً</p>
            </div>
          )}
        </div>
          ...
          <div className="mb-5 animate-fadeIn">
            ...
            <div className="relative">
              <input
                ...
                className="w-full py-4 px-4 bg-white border-2 border-gray-200 rounded-xl text-lg font-bold text-center focus:border-emerald-400 focus:outline-none"
                style={{ direction: 'ltr' }}
              />
            ...
            <div className="bg-white rounded-xl p-4 mb-5 shadow-sm border border-gray-100">
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-gray-500">المبادرة</span>
                  <span className="font-semibold text-sm">{selectedInitiative.title_ar}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">المستفيد</span>
                ...
                <div className="flex justify-between border-t pt-2 mt-2">
                  ...
                  <span className="text-xl font-bold text-emerald-600">{parseInt(amount).toLocaleString('ar-SA')} ر.س</span>
                </div>
              </div>
            </div>
...
export default InitiativePayment
