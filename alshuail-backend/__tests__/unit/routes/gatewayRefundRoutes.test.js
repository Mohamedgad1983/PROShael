...
const mockList = jest.fn((_req, res) => res.json({ success: true, data: [] }));
const mockRefund = jest.fn((_req, res) => res.json({ success: true }));
...
jest.unstable_mockModule('../../../src/controllers/gatewayRefundController.js', () => ({
  listPendingGatewayRefunds: mockList,
  refundPendingGatewayPayment: mockRefund,
}));
...
describe('gateway refund route RBAC', () => {
  test('allows a financial manager to review refund obligations', async () => {
  ...
  test('does not let a financial manager execute a refund', async () => {
    const response = await request(app)
      .post('/api/payments/gateway/pending-refunds/payment-id/refund')
      .set('x-test-role', 'financial_manager')
      .send({});
  ...
  test('allows only a super administrator to reach the refund controller', async () => {
    const response = await request(app)
      .post('/api/payments/gateway/pending-refunds/payment-id/refund')
