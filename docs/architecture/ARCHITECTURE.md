...
```
    ...
    └───────────────┘           └─────────────────┘           └────────┬────────┘
                                          ...
                                          ▼                            ▼
                                ┌─────────────────┐          ┌─────────────────┐
                                │ VPS PostgreSQL  │          │ External APIs   │
                                │ + Local Uploads │          │ - Firebase FCM  │
                                │                 │          │ - UltraMsg      │
                                │                 │          │ - Twilio        │
                                └─────────────────┘          └─────────────────┘
```
...
├── src/
│   ├── config/
│   │   ├── database.js       # PostgreSQL query builder initialization
...
│     └── Parameterized queries via PostgreSQL query builder  │
│                                                             │
│  7. XSS Prevention                                          │
...
Documents are written to the VPS filesystem through `src/config/documentStorage.js`.
The public path is `/uploads/...`, with `/api/uploads/...` kept as a compatibility
alias for older stored links.
...
│      └── Server: Nginx                                      │
│                                                             │
│  PostgreSQL 15 + Local Uploads (same VPS)                   │
│  ├── Database: self-hosted PostgreSQL                       │
│  └── Uploads: /var/www/uploads/alshuail                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
3. **Database Read Replicas**: PostgreSQL streaming replication or managed replica
