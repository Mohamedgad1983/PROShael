-- =====================================================
-- AL-SHUAIL STATEMENT SYSTEM - MATERIALIZED VIEWS
...
WITH member_balances AS (
  SELECT
    m.id,
    COALESCE(m.full_name, m.name, CONCAT(m.first_name, ' ', m.last_name), 'عضو غير مسمى') as full_name,
    m.phone,
...
BEGIN
  ...
  RAISE NOTICE 'member_statement_view contains % records', view_count;

  -- Check critical members
  SELECT COUNT(*) INTO critical_count FROM critical_members_view;
  RAISE NOTICE 'critical_members_view contains % critical members', critical_count;

  -- Show sample data
  RAISE NOTICE 'Views created successfully. Sample queries:';
